...
datetime EA_OrderOpenTime; //Order open time
datetime EA_OrderCloseTime; //Order Close time
datetime LastOrderCloseTime; // Last order close time
...
//--- to simplify the coding and speed up access data are put into internal variables
   ...
   // previous_resistance=iCustom(NULL,0,"Previous Support and Resistance",0,0);
   // previous_support=iCustom(NULL,0,"Previous Support and Resistance",1,0);
   // Print("Period ", iBars());
   // Print("Bar count on the ", Symbol()," is: ",iBars(Symbol(),PERIOD_CURRENT));
   // Print(Symbol(), "; ", OrdersTotal()," ", Bid<support, "; ",Ask>resistance, "; ",in_trade_window);
// total=OrdersTotal();
 int hstTotal=OrdersHistoryTotal();
 if(OrderSelect(hstTotal-2,SELECT_BY_POS,MODE_HISTORY)==true)
// datetime LOCT=OrderCloseTime();
 //Print("hstTotal: ",hstTotal,"; LOCT: ",LOCT, "; Last order trade time: ", TimeToStr(LOCT,TIME_DATE|TIME_SECONDS), "; PreviousDay: ",PreviousDay, "; CurrentDay: ", CurrentDay);
 // ======================================================================================
 nDayTrades=0;
...
do
  {
     if(OrderSelect(trade,SELECT_BY_POS,MODE_HISTORY)==true)
       {
         //OOT=OrderOpenTime();
         //if(OOT>0) Print("OrdersTotal: ",hstTotal,"; Open time for the order:  ", trade, " is: ", TimeToStr(OOT,TIME_DATE|TIME_SECONDS));
       ...
       Print("OrderSelect failed error code is: ",GetLastError(), "; with trade: ", trade);  
   trade--; // without this operator an infinite loop will appear!
  }
while(EA_OrderCloseTime>PreviousDay &&trade>=0);
Print("While  loop - nDayTrades: ",nDayTrades);
//=======================================================================================
...
// bool nTradeAllowance=MaxNumberDayTrades>nDayTrades;
//Print("In trade window: ",in_trade_window,"; Trade allowance ok: ",nTradeAllowance, "; MaxNumberDayTrades: ",MaxNumberDayTrades,"; Open Trades: ",symbol_total, "; nDayTrades: ",nDayTrades);
     ...
     // Print(total
      ...
      if(Ask>resistance && in_trade_window)
        ...
        //Print("SELL!");
         ...
         ticket=0; // OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,Bid+StopLoss*Point,Bid-TakeProfit*Point,"AE Capital, S&R sample",16384,0,Red);
         if(ticket>0)
           {
      ...
      if(Bid<support && in_trade_window)
        ...
        ticket=0; //OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,Ask-StopLoss*Point,Ask+TakeProfit*Point,"AE Capital, S&R sample",16384,0,Green);
